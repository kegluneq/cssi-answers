...
// Challenge #2
function toggleBoxActive(event) {
  let box = event.target;
  let isBoxActive = box.classList.contains("active");
  console.log("Changing whether the clicked box #" + box.id +
      " is active from " + isBoxActive + " to " + !isBoxActive);
  box.classList.toggle("active");
}

// Find all of the boxes in row2.
let challenge2Boxes = document.querySelectorAll("#container2 .box");
for (let i = 0; i < challenge2Boxes.length; i++) {
  challenge2Boxes[i].addEventListener("click", toggleBoxActive);
}
